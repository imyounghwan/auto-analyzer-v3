...
export async function calculateNielsenScores(htmlAnalysis, advancedMetrics = {}) {
  ...
  const scores = {
    ...
    N4_2_terminology_consistency: structure.headings.total > 0 ? 3.5 : null,  // 제목/텍스트 있어야 체크 가능
    ...
    N5_2_confirmation_dialog: structure.forms.buttons > 0 ? 3.5 : null,  // 버튼 있어야 체크 가능
    N5_3_constraints: structure.forms.inputs > 0 ? 3.5 : null,  // 입력 있어야 체크 가능
    
    // N6: 인식보다 회상
    N6_1_visible_options: structure.navigation.hasNav || structure.links.total > 0 ? 4.0 : null,  // 메뉴/링크 있어야 체크
    N6_2_recognition_cues: 3.5,
    N6_3_memory_load: structure.navigation.hasBreadcrumb ? calculateBasicScore(structure.navigation.hasBreadcrumb, 3.5) : null,
    
    // N7: 유연성과 효율성
    N7_1_accelerators: interaction.N7_1_accelerators?.score || null,  // 실측 실패 시 null
    N7_2_personalization: interaction.N7_2_personalization?.score || 3.0,
    N7_3_batch_operations: interaction.N7_3_batch_operations?.score || null,  // 실측 실패 시 null
    ...
    // N13: 콘텐츠 신선도
    N13_content_freshness: structure.meta?.date || accessibility.timeElement ? 3.5 : null,  // 날짜 정보 있어야 체크
    ...
    // N15: 파일 다운로드
    N15_file_download: structure.links.download > 0 ? 3.0 : null,  // download 링크 있어야 체크
    
    // N16: 폼 복잡도
    N16_form_complexity: structure.forms.count > 0 ? calculateFormComplexity(structure.forms) : null,
    
    // N17: 성능 (Puppeteer Performance API 실측)
    ...(performance.error ? {} : {
    ...
    // N18: 다국어 지원
    N18_multilingual: accessibility.langAttribute ? calculateBasicScore(accessibility.langAttribute, 2.5) : null,
